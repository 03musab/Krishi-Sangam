...
export default function EquipmentRental() {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    getEquipment(search ? `search=${encodeURIComponent(search)}` : '')
      .then((d) => {
        if (cancelled) return;
  ...
  return (
    <>
      <PageBanner title="Equipment Rental" color="orange" actionLabel="List Equipment" onAction={() => navigate('list-equipment')} />
      ...
      {!loading && !error && listings.length === 0 && (
        <div className="listings-empty">No equipment found.</div>
      )}
