/* Krishi Sangam — server/routes/services.js
   (Agricultural Services & Labour Team Bookings) */
...
router.post('/book', authenticateToken, (req, res) => {
  try {
    ...
    const booking = db.prepare('SELECT * FROM service_bookings WHERE id = ?')
...
router.put('/:id', authenticateToken, (req, res) => {
  try {
    ...
    db.prepare(`UPDATE service_bookings SET status = COALESCE(?, status), updated_at = datetime('now') WHERE id = ?`)
      .run(status || null, req.params.id);
    const updated = db.prepare('SELECT * FROM service_bookings WHERE id = ?').get(req.params.id);
