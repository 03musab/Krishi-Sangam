/* KrishiSetu — server/routes/payments.js (Payments / Escrow — async, Postgres) */
...
router.get('/stats', authenticateToken, requireRole('admin'), async (req, res) => {
  try {
    ...
    const heldInEscrow = await db.prepare(`SELECT COALESCE(SUM(amount), 0) as total FROM payments WHERE status = 'held'`).get();
    const totalTransactions = await db.prepare(`SELECT COUNT(*) as count FROM payments`).get();
...
router.get('/all', authenticateToken, requireRole('admin'), async (req, res) => {
  try {
    const db = getDb();
    const payments = await db.prepare(`SELECT p.*, u1.username as payer_name, u2.username as payee_name, b.listing_title, b.listing_type
...
router.get('/', authenticateToken, async (req, res) => {
  try {
    const db = getDb();
    const payments = await db.prepare(`SELECT p.*, b.listing_title, b.listing_type, b.listing_id
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await db.prepare(`INSERT INTO payments (booking_id, payer_id, payee_id, amount, status, method, transaction_ref)
      VALUES (?, ?, ?, ?, 'held', ?, ?)`).run(booking_id, booking.booker_id, booking.owner_id, booking.total_price, method || 'upi', txnRef);
    await db.prepare(`UPDATE bookings SET payment_status = 'escrow', updated_at = NOW() WHERE id = ?`).run(booking_id);
...
router.put('/:id/release', authenticateToken, async (req, res) => {
  try {
    ...
    await db.prepare(`UPDATE payments SET status = 'released' WHERE id = ?`).run(req.params.id);
    await db.prepare(`UPDATE bookings SET payment_status = 'released', updated_at = NOW() WHERE id = ?`).run(payment.booking_id);
...
router.put('/:id/refund', authenticateToken, async (req, res) => {
  try {
    ...
    if (payment.status !== 'held') return res.status(400).json({ error: 'Not in escrow.' });
    await db.prepare(`UPDATE payments SET status = 'refunded' WHERE id = ?`).run(req.params.id);
    await db.prepare(`UPDATE bookings SET payment_status = 'refunded', updated_at = NOW() WHERE id = ?`).run(payment.booking_id);
